...
class LiteRtModelSource {
  LiteRtModelSource({http.Client? client})
  ...
  Future<LiteRtCatalogEntry> resolve(String url) async {
    ...
    if (uri == null ||
        ...
        uri.hasPort ||
        parts.length < 5 ||
        !{'blob', 'resolve'}.contains(parts[2]) ||
        parts.any(
          (p) => p.isEmpty || p == '.' || p == '..' || p.contains('\\'),
        ) ||
        !parts.last.toLowerCase().endsWith('.litertlm')) {
      throw const FormatException(
        'Expected an HTTPS Hugging Face .litertlm file link',
      );
    }
    ...
    final api = Uri(
      scheme: 'https',
      host: 'huggingface.co',
      pathSegments: ['api', 'models', ...parts.take(2), 'revision', parts[3]],
    ...
    for (final item in siblings) {
      ...
      return LiteRtCatalogEntry(
        ...
        sizeBytes: size,
        license:
            (decoded['cardData'] is Map ? decoded['cardData']['license'] : null)
                ?.toString() ??
            '—',
        contextTokens: 0,
        repoUrl: Uri.https('huggingface.co', repo),
        downloadUrl: Uri(
          scheme: 'https',
          host: 'huggingface.co',
          pathSegments: [...parts.take(2), 'resolve', commit, ...parts.skip(4)],
        ),
