...
void main() {
  for (final occupied in [false, true]) {
    test('manual Codex callback works with occupied listener: $occupied', () async {
      HttpServer? blocker;
      if (occupied) {
      ...
      final client = MockClient((request) async {
        ...
        expect(request.bodyFields['redirect_uri'], 'http://localhost:1455/auth/callback');
        expect(oauthPkceChallenge(request.bodyFields['code_verifier']!),
      ...
      final saved = await ChatGptOAuthAdapter().login(
        OAuthWire(client), cancellation,
        (value) async {
          prompt = value;
          expect(value.submitAuthorizationCode, isNotNull,
        ...
        launcher: (url) async {
          ...
          final valid = Uri.parse('http://localhost:1455/auth/callback').replace(
            queryParameters: {'code': 'synthetic-code', 'state': url.queryParameters['state']!},
          );
          for (final invalid in [
            ...
            valid.replace(query: 'code=synthetic-code&state=old-login').toString(),
  ...
  test('validated loopback does not depend on a second Android intent', () async {
    ...
    final pending = callback.authorize(
      Uri.parse('https://auth.openai.com/oauth/authorize?state=synthetic-state'),
      const Duration(seconds: 3), (_) async => true,
    );
    final outcome = expectLater(
      pending.timeout(const Duration(seconds: 1)),
      completion(isA<Uri>().having((uri) => uri.queryParameters['code'], 'code', 'synthetic-code')),
    );
    await browser.launched.future;
    final request = await client.getUrl(callback.redirectUri.replace(
      queryParameters: {'code': 'synthetic-code', 'state': 'synthetic-state'},
    ));
    request.followRedirects = false;
    final response = await request.close();
    expect(response.headers.value('location'), isNot(contains('synthetic-code')));
    await response.drain<void>();
    await outcome;
  });

  test('device network failure reports safe stage and cause, never credentials', () async {
    ...
    addTearDown(client.close);
    await expectLater(
      ChatGptOAuthAdapter().login(OAuthWire(client), OAuthCancellation(), (_) async {}),
      throwsA(isA<ProviderOAuthException>()
          .having((e) => e.kind, 'kind', ProviderOAuthFailure.network)
          .having((e) => e.code, 'safe diagnostic', 'device-authorization/dns')
          .having((e) => '${e.message} ${e.code}', 'no credentials',
              allOf(isNot(contains('DO-NOT-LOG')), isNot(contains('PRIVATE'))))),
    );
  });
}
