...
void main() {
  ...
  test('loadModel sends the exact args and returns the actual backend', () async {
  ...
  test('events decode to the right typed variant, keyed by requestId', () async {
    ...
    expect(received[1], isA<LiteRtDone>().having((e) => e.requestId, 'requestId', 'r1'));
    expect(
      received[2],
      isA<LiteRtError>()
    ...
    expect(
      received[3],
      isA<LiteRtEngineStateChanged>().having((e) => e.state, 'state', 'generating'),
    );
  });
