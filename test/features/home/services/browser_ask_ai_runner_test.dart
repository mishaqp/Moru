...
void main() {
  ...
  test(
    'onTaskStarted reports the generationRunId (not the assistantMessageId) '
    'once send() confirms the run started, well before any terminal event',
    () async {
      final bridge = BrowserAskAiBridge();
      addTearDown(bridge.dispose);
      final terminal = StreamController<GenerationTerminalEvent>.broadcast();
      ...
      unawaited(
        runBrowserAskAiRequest(
          bridge: bridge,
          request: const BrowserAskAiRequest(id: 'req-started', text: 'hi'),
          currentConversationId: conversation.id,
          getConversation: (id) => id == conversation.id ? conversation : null,
          getAssistantById: (id) => id == assistant.id ? assistant : null,
          currentAssistant: null,
          ...
          send:
              ({
                ...
                onGenerationStarted('message-started');
                return ChatActionResult.success(
                  placeholderMessage('message-started'),
                  generationRunId: 'run-started',
                );
              },
          cancel: (_, {expectedMessageId}) async {},
        ),
      ...
      await pumpEventQueue();
      // Reported before the terminal event, not after.
      expect(started, [('run-started', conversation.id)]);

      terminal.add(completedEvent('message-started', 'done'));
      await pumpEventQueue();
      expect(started, [('run-started', conversation.id)]);
    },
  );
