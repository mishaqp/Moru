...
void main() {
  ...
  test('speech model names and descriptions are Russian without changing runtime metadata', () async {
    final ru = await AppLocalizations.delegate.load(const Locale('ru'));
    final originals = [
      for (final model in SherpaModelCatalog.models)
        (model.id, model.archiveUri, model.archiveRoot, model.requiredFiles.toList()),
    ];
    for (final model in SherpaModelCatalog.models) {
      expect(model.localizedName(ru), matches(RegExp('[А-Яа-яЁё]')));
      expect(model.localizedDescription(ru), matches(RegExp('[А-Яа-яЁё]')));
      expect(model.localizedDescription(ru), isNot(matches(RegExp('[\u4e00-\u9fff]'))));
    }
  ...
  test('speech model descriptions support English and preserve Chinese catalog presentation', () async {
    final en = await AppLocalizations.delegate.load(const Locale('en'));
    final zh = await AppLocalizations.delegate.load(const Locale('zh'));
    for (final model in SherpaModelCatalog.models) {
      expect(model.localizedDescription(en), isNot(matches(RegExp('[\u4e00-\u9fff]'))));
  ...
  for (final locale in ['ru', 'en']) {
    testWidgets('code-fence fallback uses $locale while explicit language stays unchanged', (tester) async {
      await tester.binding.setSurfaceSize(const Size(440, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      final harness = await createBusinessTestHarness(initial: {'app_locale_v1': locale});
      ...
      await tester.pumpWidget(ChangeNotifierProvider.value(
        ...
        child: MaterialApp(
          ...
          home: const Scaffold(body: SingleChildScrollView(child: MarkdownWithCodeHighlight(
            text: '```\nplain sample\n```\n\n```python\nprint(42)\n```',
          ))),
        ),
      ));
