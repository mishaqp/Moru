...
void main() {
  ...
  test(
    'notification presentation switches RU to EN without changing payload IDs',
    () async {
      ...
      expect(
        NotificationService.conversationIdFromPayload('chat-complete:$conversation'),
        conversation,
      );
      await NotificationService.configureLocalizations(ru);
    },
  ...
  test(
    'all built-in palettes have RU names while IDs and English stay intact',
    () async {
      final ru = await AppLocalizations.delegate.load(const Locale('ru'));
      final en = await AppLocalizations.delegate.load(const Locale('en'));
  ...
  testWidgets(
    'Android display settings offer Russian and retain a chosen English locale',
    (tester) async {
