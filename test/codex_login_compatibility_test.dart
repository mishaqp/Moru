...
void main() {
  group('Codex ChatGPT subscription login', () {
    for (final field in ['user_code', 'usercode']) {
      test('completes device login with $field and keeps the Pro plan', () async {
        ...
        expect(prompts.single.url.toString(), 'https://auth.openai.com/codex/device');
    ...
    test('accepts access-token exp when expires_in is absent', () {
      final credentials = ChatGptOAuthAdapter().credentials(_tokens(expiresIn: false));
    ...
    test('refresh without expires_in keeps the session and rotates the token', () async {
    ...
    test('still rejects credentials without any usable expiry', () {
      expect(
        ...
        throwsA(isA<ProviderOAuthException>().having(
          (error) => error.kind, 'kind', ProviderOAuthFailure.invalidResponse,
        )),
      );
    });
